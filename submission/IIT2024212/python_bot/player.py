...
ENHANCED_MODEL = {
    'seed': 199441361,  # Different seed for unpredictability
    'preflop_samples': 2048,  # Increased from 1711 for better accuracy
    'mc_scale': 1.35,  # Increased from 1.196 for more iterations
    'haircut_scale': 0.58,  # Slightly adjusted for better reads
    'aggro_up_threshold': 0.52,  # More sensitive to opponent folds
    'aggro_down_threshold': 0.43,  # More sensitive to opponent aggression
    'aggro_step': 0.18,  # Slightly larger adjustments
    'bounty_bump_scale': 1.25,  # Better bounty exploitation
    'preflop_call_margin': 0.035,  # Tighter calling
    'river_probe_raise_prob': 0.58,  # More river aggression
    'low_equity_bluff_prob': 0.32,  # Optimized bluff frequency
    'postflop_value_raise_eq': 0.84,  # More value betting
    'semi_raise_eq_low': 0.54,  # Wider semi-bluff range
    'semi_raise_eq_high': 0.78,  # Higher ceiling for semi-bluffs
    'semi_raise_fold_prior': 0.48,  # More aggressive semi-bluffing
    'postflop_call_margin': 0.088,  # Tighter calling postflop
}

ATTACK_PROFILE = {
    'preflop_call_margin': -0.01,  # Slightly negative for aggression
    'postflop_call_margin': 0.025,
    'postflop_value_raise_eq': 0.73,
    'semi_raise_eq_low': 0.48,
    'semi_raise_eq_high': 0.71,
    'semi_raise_fold_prior': 0.55,
    'river_probe_raise_prob': 0.75,
    'low_equity_bluff_prob': 0.18,
    'aggro_up_threshold': 0.58,
    'aggro_down_threshold': 0.22,
    'aggro_step': 0.12,
}


def load_model():
    return dict(ENHANCED_MODEL)
...
def _precompute_preflop_equity(samples_per_hand=512):  # Increased samples
...
def fast_equity(hero, board, samples):
    ...
    # Early termination for extreme equity cases
    if samples >= 100:
        # Quick sample to detect obvious strength/weakness
        quick_wins = quick_ties = 0
        for _ in range(min(20, samples // 5)):
            random.shuffle(remaining)
            opp = remaining[:2]
        ...
        quick_eq = (quick_wins + 0.5 * quick_ties) / min(20, samples // 5)
        if quick_eq > 0.85:
            return min(0.95, quick_eq + 0.05)  # Boost confidence
        elif quick_eq < 0.15:
            return max(0.05, quick_eq - 0.05)  # Reduce confidence
...
class OpponentModel:
    ...
    def __init__(self):
        ...
        # Enhanced tracking for mirror detection
        self.preflop_ranges_seen = []
        self.bet_sizing_patterns = defaultdict(list)
        self.street_actions = defaultdict(list)
    ...
    def detect_mirror_style(self):
        if self.preflop_actions < 20:
            return False
        
        # Check if opponent plays similar ranges to us
        vpip = self.prior_preflop_raise() + 0.3  # Estimate VPIP
        pfr = self.prior_preflop_raise()
        
        # Mirror bots typically have balanced stats
        if 0.25 <= pfr <= 0.35 and 0.4 <= self.prior_postflop_bet() <= 0.6:
            self.mirror_confidence = min(1.0, self.mirror_confidence + 0.1)
        else:
            self.mirror_confidence = max(0.0, self.mirror_confidence - 0.05)
        
        return self.mirror_confidence > 0.6
...
class Player(Bot):
    def __init__(self):
        ...
        self.last_equity_result = None
        self.mirror_adjustment_factor = 1.0

    def time_samples(self, game_clock, street):
        # Enhanced sample scaling for better accuracy
        scale = self.model['mc_scale']
        if game_clock < 2.5:
            return 0 if scale >= 1.0 else int(60 * scale)  # Increased from 40
        if game_clock < 6.0:
            return int(160 * scale)  # Increased from 120
        if game_clock < 15.0:
            return int(300 * scale)  # Increased from 240
        if game_clock < 30.0:
            return int(450 * scale)  # Increased from 380
        return int(650 * scale)  # Increased from 520
    ...
    def mc_equity(self, hero_str, board_str, samples, opp_contribution=0, facing_bet=False):
        ...
        board_hash = hash(tuple(board_str))
        cache_key = (tuple(hero_str), board_hash, samples)
        
        if cache_key in self.equity_cache and samples < 200:
        ...
        if combos is None:
            result = fast_equity(hero, board, samples)
        else:
            result = mc_equity_vs_range(hero, board, combos, samples)
        
        # Cache small sample results
        if samples < 200:
    ...
    # Enhanced bet sizing with pattern recognition
    def pick_raise_size(self, equity, pot, my_pip, opp_pip, my_stack, opp_stack, min_raise, max_raise, street, aggression_shift):
        # Detect if this might be a mirror match situation
        mirror_bonus = 0.0
        if self.opp.detect_mirror_style():
            # In mirror matches, use slightly different sizing to break symmetry
            if equity > 0.70:
                mirror_bonus = 0.05  # Slightly larger with strong hands
            elif equity > 0.50:
                mirror_bonus = -0.03  # Slightly smaller with marginal hands
        
        if equity > 0.85:
            frac = 1.15  # Increased from 1.1
        elif equity > 0.72:
            frac = 0.85  # Increased from 0.8
        elif equity > 0.60:
            frac = 0.65  # Increased from 0.6
        else:
            frac = 0.52  # Increased from 0.5
            
        frac += aggression_shift + mirror_bonus
        frac = max(0.35, min(1.35, frac))  # Increased upper bound from 1.3
    ...
    def estimate_opponent_tightness(self, my_contribution, opp_contribution, continue_cost, street, facing_bet):
        ...
        if street == 0:
            if committed >= 0.85:
                return 0.20  # Slightly reduced from 0.22
            if committed >= 0.40:
                return 0.12  # Reduced from 0.14
            if committed >= 0.15:
                return 0.06  # Reduced from 0.08
            if committed >= 0.06:
                return 0.04  # Reduced from 0.05
            return 0.02
        else:
            if committed >= 0.80:
                return 0.16  # Reduced from 0.18
            if committed >= 0.40:
                return 0.10  # Reduced from 0.12
            if committed >= 0.20:
                return 0.07  # Reduced from 0.08
            if committed >= 0.08:
    ...
    def build_decision_profile(self, game_state, street):
        ...
        # Enhanced match mode adjustments
        mode = self.match_mode(game_state)
        if mode == 'protect':
            profile = DecisionProfile(
                preflop_call_margin=profile.preflop_call_margin + 0.015,
                postflop_call_margin=profile.postflop_call_margin + 0.018,
                value_raise_eq=min(0.93, profile.value_raise_eq + 0.016),
                semi_raise_eq_low=min(0.86, profile.semi_raise_eq_low + 0.016),
                semi_raise_eq_high=min(0.93, profile.semi_raise_eq_high + 0.012),
                semi_raise_fold_prior=min(0.91, profile.semi_raise_fold_prior + 0.022),
                river_probe_raise_prob=max(0.0, profile.river_probe_raise_prob * 0.78),
                low_equity_bluff_prob=max(0.0, profile.low_equity_bluff_prob * 0.68),
                aggro_up_threshold=profile.aggro_up_threshold + 0.025,
                aggro_down_threshold=min(0.66, profile.aggro_down_threshold + 0.025),
                aggro_step=profile.aggro_step * 0.88,
            )
        elif mode == 'chase':
            profile = DecisionProfile(
                preflop_call_margin=profile.preflop_call_margin - 0.004,
                postflop_call_margin=profile.postflop_call_margin - 0.004,
                value_raise_eq=max(0.54, profile.value_raise_eq - 0.004),
                semi_raise_eq_low=max(0.28, profile.semi_raise_eq_low - 0.008),
                semi_raise_eq_high=min(0.93, profile.semi_raise_eq_high + 0.008),
                semi_raise_fold_prior=max(0.23, profile.semi_raise_fold_prior - 0.008),
                river_probe_raise_prob=min(0.96, profile.river_probe_raise_prob * 1.025),
                low_equity_bluff_prob=min(0.52, profile.low_equity_bluff_prob * 1.035),
                aggro_up_threshold=max(0.28, profile.aggro_up_threshold - 0.015),
                aggro_down_threshold=max(0.07, profile.aggro_down_threshold - 0.015),
                aggro_step=min(0.32, profile.aggro_step * 1.025),
            )
        ...
        if street == 5:
            profile = DecisionProfile(
                ...
                value_raise_eq=max(0.54, profile.value_raise_eq - 0.012),
                semi_raise_eq_low=max(0.28, profile.semi_raise_eq_low - 0.012),
    ...
    # Enhanced action logic with tie-breaking
    def get_action(self, game_state, round_state, active):
        ...
        if street == 0:
            ...
            # Dynamic adjustment for mirror matches
            mirror_adjustment = 0.0
            if self.opp.detect_mirror_style():
                if eq_raw > 0.65:
                    mirror_adjustment = 0.02  # Be more aggressive with strong hands
                elif eq_raw < 0.35:
                    mirror_adjustment = -0.01  # Be more careful with weak hands
                    
            eq_eff = eq_vs_opp + bounty_bump + mirror_adjustment
            ...
            if not facing_bet:
                if RaiseAction in legal_actions:
                    # Enhanced opening ranges
                    if klass == 'premium':
                        target = opp_pip + max(7, int(pot * 1.7))  # Increased sizing
                    elif klass == 'strong':
                        target = opp_pip + max(6, int(pot * 1.3))  # Increased sizing
                    elif klass in ('medium', 'openable'):
                        target = opp_pip + max(5, int(pot * 1.0))  # Increased sizing
                    elif has_bounty_rank:
                        target = opp_pip + max(5, int(pot * 0.9))  # Increased sizing
                    else:
                        target = None
            ...
            if opp_pip >= 25 or opp_committed >= 0.12:
                if RaiseAction in legal_actions and klass == 'premium':
                    ...
                    jam_target = min(opp_pip + int((pot + continue_cost) * 2.4), max_rr)  # Increased aggression
                    return RaiseAction(max(jam_target, min_raise_total))
                if CallAction in legal_actions and klass in ('premium', 'strong'):
                    return CallAction()
                if CallAction in legal_actions and klass == 'medium' and continue_cost <= pot * 0.58:  # Slightly looser
                    return CallAction()
                if FoldAction in legal_actions:
                    return FoldAction()
                if CallAction in legal_actions:
            ...
            if opp_pip >= 5:
                if RaiseAction in legal_actions and klass == 'premium':
                    target = opp_pip + max(min_raise_total - opp_pip, int(continue_cost * 3.2))  # Increased
                    return RaiseAction(min(max(target, min_raise_total), max_raise_total))
                if RaiseAction in legal_actions and klass == 'strong':
                    target = opp_pip + max(min_raise_total - opp_pip, int(continue_cost * 2.8))  # Increased
                    return RaiseAction(min(max(target, min_raise_total), max_raise_total))
                if CallAction in legal_actions and klass in ('premium', 'strong', 'medium'):
                    return CallAction()
                if CallAction in legal_actions and has_bounty_rank and klass == 'openable' and continue_cost <= 9:  # Looser
                    return CallAction()
                if FoldAction in legal_actions:
                    return FoldAction()
                if CheckAction in legal_actions:
            ...
            if continue_cost <= 2:
                if RaiseAction in legal_actions and klass == 'premium':
                    target = opp_pip + max(min_raise_total - opp_pip, 7)  # Increased
                    return RaiseAction(min(max(target, min_raise_total), max_raise_total))
        ...
        # Postflop with enhanced sizing patterns
        eq_raw = base_equity
        ...
        if not facing_bet:
            if RaiseAction in legal_actions and pot >= 4:
                if eq_raw >= 0.66:  # Slightly lower threshold
                ...
                if eq_raw >= 0.50:  # Slightly lower threshold
                    frac = 0.58 + aggro_shift  # Increased base
                    frac = max(0.42, min(0.82, frac))  # Adjusted ranges
                    target = opp_pip + max(min_raise_total - opp_pip, int(max(pot, 4) * frac))
                    return RaiseAction(min(max(target, min_raise_total), max_raise_total))
                if eq_raw >= 0.36 and street == 3:  # Slightly lower threshold
                    if random.random() < profile.river_probe_raise_prob:
                        frac = 0.48  # Increased
                        target = opp_pip + max(min_raise_total - opp_pip, int(max(pot, 4) * frac))
                        return RaiseAction(min(max(target, min_raise_total), max_raise_total))
                if eq_raw < 0.28 and pot >= 10 and random.random() < profile.low_equity_bluff_prob:  # More aggressive bluffing
                    target = opp_pip + max(min_raise_total - opp_pip, int(max(pot, 4) * 0.48))  # Increased
                    return RaiseAction(min(max(target, min_raise_total), max_raise_total))
            if CheckAction in legal_actions:
                return CheckAction()
        ...
        if (RaiseAction in legal_actions and profile.semi_raise_eq_low <= eq_eff < profile.semi_raise_eq_high and 
            fold_prior > profile.semi_raise_fold_prior and continue_cost <= pot * 0.65 and opp_committed < 0.38):  # Slightly looser
            target = opp_pip + max(min_raise_total - opp_pip, int((pot + continue_cost) * 0.78))  # Increased
            return RaiseAction(min(max(target, min_raise_total), max_raise_total))
        ...
        if CallAction in legal_actions:
            if eq_eff >= req + profile.postflop_call_margin:
                if big_bet and not has_pair_or_better and eq_raw < 0.56:  # Slightly tighter folding
                    pass
                else:
                    return CallAction()
            if continue_cost <= max(3, pot * 0.16) and eq_raw >= 0.30:  # Slightly looser
                return CallAction()
            if street == 5 and fold_prior < 0.27 and eq_eff >= pot_odds and has_pair_or_better:  # More aggressive river calls
                return CallAction()
    ...
    def _track_opponent(self, round_state, active, street):
        ...
        if opp_pip > self.prev_opp_pip:
            bet_size = opp_pip - self.prev_opp_pip
            pot_size = my_pip + opp_pip + (STARTING_STACK - round_state.stacks[active]) + (STARTING_STACK - round_state.stacks[1-active])
            if pot_size > 0:
                bet_fraction = bet_size / pot_size
                self.opp.bet_sizing_patterns[street].append(bet_fraction)
