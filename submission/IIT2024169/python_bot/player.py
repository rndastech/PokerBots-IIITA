'''
=============================================================
  REFINED BOUNTY HOLD'EM POKER BOT v3.0 -- newplayer
=============================================================
...
def classify_hand_strength(my_cards, board_cards):
    ...
    try:
        my_hand = [eval7.Card(c) for c in my_cards]
        ...
        category = HAND_RANK_NAMES.get(hand_type, 'unknown')
        deck = eval7.Deck()
...
class WindowedOpponentModel:
    ...
    def record_action(self, action_str, street, pip_amount, pot_size):
        if action_str == 'raise':
            ...
            if pot_size > 0 and pip_amount > 0:
                self._round['sizes'].append(pip_amount / pot_size)
    ...
    def _stats(self, records):
        ...
        return {'vpip': vpip, 'pfr': pfr, 'af': af, 'pf_fold': pff,
                'avg_sz': avg_sz, 'cbet': cbet, 'sd_rate': sd, 'jam': jm, 'n': n}
...
class Player(Bot):
    '''Refined Bounty Hold'em bot v3.0.'''
    ...
    def get_action(self, game_state, round_state, active):
        ...
        # Late-game variance shield: lock up solid leads instead of reopening
        # the match with marginal calls and bluffs.
        round_num = game_state.round_num
        rounds_left = max(1, NUM_ROUNDS - round_num)
        elastic_threshold = max(100, rounds_left * 2)
        if ((round_num >= 850 and bankroll >= elastic_threshold) or
                (round_num >= 900 and bankroll >= 200) or
                (round_num >= 800 and bankroll >= 600)):
        ...
        if street == 0:
            ...
            if big and EVAL7_AVAILABLE and clock > 5.0:
                it = 200
                if clock < 8.0:
                    it = 80
                elif clock < 15.0:
        ...
        protect_lead = bankroll > 80 or (phase == 'late' and bankroll > 0)
        qualification_mode = baseline_safe or (provisional_honest and protect_lead)
    ...
    def _preflop(self, legal, cards, bounty, eq, pot_od, cost, pot,
        ...
        if cost == 0:
            ...
            if not is_bb:
                ...
                if eq >= 0.36 and RaiseAction in legal and m < 0.25:
        ...
        if cost > 0:
            ...
            if cost >= STARTING_STACK * 0.80:
                min_tier = 2
                if bankroll > 80 or phase == 'late':
                    min_tier = 3
                if jam_tier >= min_tier and CallAction in legal:
                    return CallAction()
                if FoldAction in legal:
                    return FoldAction()
    ...
    def _postflop(self, legal, cards, board, bounty, eq, pct, pot_od,
        ...
        if pct >= 0.88 and tier != 'monster':
            tier = 'monster'
        elif pct >= 0.78 and tier in ('medium', 'drawing', 'marginal', 'weak'):
            tier = 'very_strong'
        ...
        if cost == 0:
            return self._pf_nobet(
                ...
                losing, fb, tb, qualification_mode)
        else:
            return self._pf_bet(
                ...
                qualification_mode, provisional_honest)
    ...
    def _pf_bet(self, legal, cards, board, eq, pct, pot_od, adj_po,
        ...
        if tier == 'very_strong':
            if qualification_mode and provisional_honest and street >= 4:
                if CallAction in legal:
                    return CallAction()
            if cost < pot * 0.7 and RaiseAction in legal:
        ...
        if tier == 'medium':
            ...
            if street >= 4 and od in ('unknown', 'tag'):
                ...
                if cost > tl:
                    if od == 'unknown' and eq >= 0.50 and cost <= pot * 0.35:
                        if CallAction in legal:
                            return CallAction()
                    ...
                    if eq < 0.55:
                        if FoldAction in legal:
                            return FoldAction()
            if street >= 4 and od in ('unknown', 'lag', 'maniac') and not provisional_honest:
                if eq >= 0.47 and cost <= pot * 0.55:
                    if CallAction in legal:
                        return CallAction()
            if self.opp.is_honest_bettor() and street >= 4:
                if self.opp.hands_played < 80 and eq >= 0.50 and cost <= pot * 0.35:
                    if CallAction in legal:
                        return CallAction()
            ...
            if cost <= cl and eq > adj_po * 0.85:
                if CallAction in legal:
                    return CallAction()
            if street == 5 and is_agg and eq > 0.38:
                if CallAction in legal:
                    return CallAction()
            if FoldAction in legal:
                return FoldAction()
        ...
        if tier == 'drawing':
            if street <= 4:
                ...
                if eq > io or (mdf_def and cost <= pot * 0.45):
                    if CallAction in legal:
                        return CallAction()
                if not is_nit and RaiseAction in legal and mr < 0.20 and cost < pot * 0.3:
                    return RaiseAction(mn)
            if street == 5:
                if pct >= 0.85:
                    if RaiseAction in legal:
