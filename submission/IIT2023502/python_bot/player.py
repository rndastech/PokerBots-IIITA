...
class OpponentModel:
    ...
    def hand_range_weight(self, raises_in_hand):
        ...
        if raises_in_hand == 2:
            return 0.35 if self.is_tight() else 0.35
        return 0.15
...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        random.seed(hash(f"{game_state.round_num}_{round_state.street}_{active}"))
        # ---------------------------------------------------------------
        ...
        if continue_cost <= 0:
            if RaiseAction in legal_actions:
                ...
                if win_prob >= 0.61 and street >= 3:
                    frac = 0.65 if texture.get("dry") else 0.50
                    return RaiseAction(self._raise_amount(round_state, active, frac, pot_size))

                if win_prob >= 0.52 and in_position and street == 3:
